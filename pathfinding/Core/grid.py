...
class Grid(object):
    ...
    def get_walkable_neighbors(self, node, radius_nodes, diagonal_movement=DiagonalMovement.never):
        ...
        LOG.debug("Step: " + str(node.step) + " neighbors before:" + ' '.join(str(n) for n in neighbors ))
            ...
            for neighbor in neighbors:
                ...
                for radius_node in radius_nodes:
                    if neighbor.x == radius_node.x and neighbor.y == radius_node.y:
                        LOG.debug("removing dangerous:" + str(neighbor))
                        is_good = False
                        break
                if is_good:
        ...
        LOG.debug("Step: " + str(node.step) + " neighbors after:  " + ' '.join(str(n) for n in good_neighbors))
